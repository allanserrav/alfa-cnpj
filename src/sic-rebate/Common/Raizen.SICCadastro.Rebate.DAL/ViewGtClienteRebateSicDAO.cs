#region Cabeçalho do Arquivo
// <Summary>
// File Name		    : ViewGtClienteRebateSicDAO.cs
// Class Name	        : ViewGtClienteRebateSicDAO
// Author		        : Leandro A. Morelato / Paulo Gerage
// Creation Date 	    : 08/02/2013
// </Summary>

// <RevisionHistory>
// <SNo value=1>
//	Modified By             : 
//	Date of Modification    : 
//	Reason for modification : 
//	Modification Done       : 
//	Defect Id (If any)      : 
// <SNo>
// </RevisionHistory>
#endregion

#region Namespaces
using System;
using System.Collections.Generic;
using System.Data;
using System.Data.Common;
using System.Security;
using System.Text;
using COSAN.Framework.DBUtil;
using Raizen.SICCadastro.Rebate.Model;
#endregion Namespaces

namespace Raizen.SICCadastro.Rebate.DAL
{
	#region classe concreta ViewGtClienteRebateSicDAO
	/// <summary>
	/// Representa funcionalidade relacionada a ViewGtClienteRebateSicDAO
	/// </summary>
	internal partial class ViewGtClienteRebateSicDAO : IViewGtClienteRebateSicDAO
	{
		#region Constantes
		/// <summary>
		/// Representa ordenação padrão da query Selecionar
		/// </summary>
		public const string orderByDefault = "";
		#endregion  Constantes de TbViewGtClienteRebateSic
		
		#region Queries
		#region Query para Selecionar registros
		/// <summary>
		/// String com a query de seleção de registros
		/// </summary>
		private string querySelecionar = new StringBuilder().Append("SELECT {0}")
		.Append(" VIEW_GT_CLIENTE_REBATE_SIC.NM_GTLOJA_CLIENTE_SIC")
		.Append(" FROM VIEW_GT_CLIENTE_REBATE_SIC")
		.Append(" {1}")
		.Append(" {2}")
		.Append("").ToString();
		#endregion Query para Selecionar registros
		#endregion Queries
		
		#region Metodos Publicos
		#region Selecionar
		/// <summary>
		/// Selecionar os dados de ViewGtClienteRebateSic
		/// </summary>
		/// <param name="viewGtClienteRebateSic">Instância de <see cref="ViewGtClienteRebateSic"/> para filtrar os dados</param>
		/// <param name="numeroLinhas">Número de linhas para ser trazidos ou 0 para todos.</param>
		/// <param name="ordem">Ordem dos dados retornados ou branco/nulo para ordem padrão</param>
		/// <returns>Retorna lista de ViewGtClienteRebateSic</returns>
		public IList<ViewGtClienteRebateSic> Selecionar(ViewGtClienteRebateSic viewGtClienteRebateSic, int numeroLinhas, string ordem)
		{
			IList<ViewGtClienteRebateSic> listViewGtClienteRebateSic = new List<ViewGtClienteRebateSic>();
			using (DatabaseManager databaseManager = new DatabaseManager("SICCadastro"))
			{
				string where = "";
				IList<DbParameter> parametros = CriarParametrosSelecionar(databaseManager, viewGtClienteRebateSic, out where);
				string newQuery = string.Format(querySelecionar,
				    (numeroLinhas > 0) ? "top " + numeroLinhas : String.Empty,
				    (string.IsNullOrEmpty(where)) ? String.Empty : "WHERE " + where,
				    (string.IsNullOrEmpty(ordem) && string.IsNullOrEmpty(orderByDefault)) ? String.Empty : ("ORDER BY " + ((string.IsNullOrEmpty(ordem)) ? orderByDefault : ordem)));
				using (SafeDataReader dbDataReader = (SafeDataReader)databaseManager.GetsDataReader(newQuery, parametros))
				{
					while (dbDataReader.Read())
					{
						listViewGtClienteRebateSic.Add(Preencher(dbDataReader));
					}
				}
				databaseManager.CloseConnection();
			}
			return listViewGtClienteRebateSic;
		}
		#endregion Selecionar
		#endregion Metodos Publicos
		
		#region Metodos Privados
		#region Metodos Gerais
		#region Preencher
		/// <summary>
		/// Preenche o objeto ViewGtClienteRebateSic a partir do SafeDataReader.
		/// </summary>
		/// <param name="reader">Instance of <see cref="SafeDataReader"/></param>
		/// <returns>O objeto ViewGtClienteRebateSic preenchido</returns>
		protected ViewGtClienteRebateSic Preencher(SafeDataReader reader)
		{
			if (reader == null) throw (new ArgumentNullException());
			ViewGtClienteRebateSic viewGtClienteRebateSic = new ViewGtClienteRebateSic();
			viewGtClienteRebateSic.NmGtlojaClienteSic = reader.GetString(C_NmGtlojaClienteSic);
			return viewGtClienteRebateSic;
		}
		#endregion Preencher
		#endregion Common Methods
		
		#region Criar Parametros
		#region Criar Parametros Selecionar
		/// <summary>
		/// Criar Parametros Selecionar
		/// </summary>
		/// <param name="databaseManager">Instance of <see cref="DatabaseManager"/></param>
		/// <param name="viewGtClienteRebateSic">Instance of <see cref="ViewGtClienteRebateSic"/></param>
		/// <returns>Lista de DbParameter</returns>
		private IList<DbParameter> CriarParametrosSelecionar(DatabaseManager databaseManager, ViewGtClienteRebateSic viewGtClienteRebateSic, out string where)
		{
			List<DbParameter> dbParams = new List<DbParameter>();
			where = "";
			if (viewGtClienteRebateSic.NmGtlojaClienteSic != null) dbParams.Add(databaseManager.CreateWhereParameter(DbType.String, "VIEW_GT_CLIENTE_REBATE_SIC", C_NmGtlojaClienteSic, DatabaseManager.SQLOperation.Like, "%" + viewGtClienteRebateSic.NmGtlojaClienteSic + "%", ref where));
			return dbParams;
		}
		#endregion Criar Parametros Selecionar
		#endregion Criar Parametros
		#endregion Metodos Privados
	}
	#endregion classe concreta 
}
